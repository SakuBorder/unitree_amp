...
class AMP_PPO:
    ...
    amp_data : Optional[Any]
        Dataset-like object providing batches of expert motion data.
    amp_normalizer : Optional[Any]
        Optional normalizer to preprocess AMP observations before they are fed into the discriminator.
    normalize_amp_input : bool, default=True
        Whether to normalize AMP inputs. Can be disabled for faster training.
    ...
    def __init__(
        ...
        # AMP normalization control - aligned with TokenHSI
        self.normalize_amp_input: bool = normalize_amp_input
        ...
        self.amp_data: Optional[Any] = amp_data
        ...
        # Reduced weight decay following optimization suggestions
        params = [
            {"params": self.actor_critic.parameters(), "name": "actor_critic"},
            {
                "params": self.discriminator.trunk.parameters(),
                "weight_decay": 1e-4,  # Reduced from 10e-4
                "name": "amp_trunk",
            },
            {
                "params": self.discriminator.linear.parameters(),
                "weight_decay": 1e-3,  # Reduced from 10e-2
                "name": "amp_head",
            },
        ]
    ...
    def init_storage(
        ...
        """
        # The standard RolloutStorage used by PPO does not accept the
        # `training_type` and `rnd_state_shape` keywords that appear in
        # some legacy AMP forks. Construct it using the canonical
        # argument list so AMP_PPO can run with the storage
        # implementation shipped with rsl_rl.
    ...
    def set_eval(self) -> None:
        """
        Sets models to evaluation mode - aligned with TokenHSI.
        In eval mode, normalizer stops updating statistics.
        """
        self.actor_critic.eval()
        self.discriminator.eval()
        if self.amp_normalizer is not None and hasattr(self.amp_normalizer, 'eval'):
            self.amp_normalizer.eval()

    def set_train(self) -> None:
        """
        Sets models to training mode - aligned with TokenHSI.
        In train mode, normalizer updates statistics.
        """
        self.actor_critic.train()
        self.discriminator.train()
        if self.amp_normalizer is not None and hasattr(self.amp_normalizer, 'train'):
    ...
    def act(self, obs: torch.Tensor, critic_obs: torch.Tensor) -> torch.Tensor:
        ...
        # Use the canonical field name expected by RolloutStorage so the
        # critic observations are written correctly when the transition is
        # added to the buffer.
    ...
    def _preproc_amp_obs(self, amp_obs: torch.Tensor) -> torch.Tensor:
        """
        Preprocess AMP observations - aligned with TokenHSI approach.
        Only normalize if enabled and normalizer is available.
        
        Parameters
        ----------
        amp_obs : torch.Tensor
            Raw AMP observations
            
        Returns
        -------
        torch.Tensor
            Preprocessed AMP observations
        """
        if not self.normalize_amp_input or self.amp_normalizer is None:
            return amp_obs
            
        return self.amp_normalizer.normalize(amp_obs)

    def update(self) -> Tuple[float, float, float, float, float, float, float, float, float, float, float]:
        """
        ...
        tuple
            ...
            mean_kl_divergence, mean_disc_agent_acc, mean_disc_demo_acc)
        """
        ...
        # TokenHSI-style accuracy metrics
        mean_disc_agent_acc: float = 0.0
        mean_disc_demo_acc: float = 0.0
        mean_discriminator_separation: float = 0.0
        ...
        if self.amp_data is not None:
            ...
            iterator = zip(generator, amp_policy_generator, amp_expert_generator)
        else:
            iterator = ((sample, None, None) for sample in generator)

        # Loop over mini-batches from the environment transitions and AMP data.
        for sample, sample_amp_policy, sample_amp_expert in iterator:
            (
                obs_batch,
                critic_obs_batch,
                actions_batch,
            ...
            if sample_amp_expert is not None and sample_amp_policy is not None:
                # Process AMP loss by unpacking policy and expert AMP samples.
                policy_state, policy_next_state = sample_amp_policy
                expert_state, expert_next_state = sample_amp_expert

                # Optimized normalization - batch process to reduce overhead
                if self.normalize_amp_input and self.amp_normalizer is not None:
                    expected = self.amp_normalizer.mean.shape[0]
                    if (
                        policy_state.shape[1] != expected
                        or expert_state.shape[1] != expected
                    ):
                        raise ValueError(
                            "AMP observation size mismatch: policy has "
                            f"{policy_state.shape[1]}, expert has {expert_state.shape[1]}, "
                            f"expected {expected}. Ensure the dataset uses the same "
                            "observation layout as the environment."
                        )
                    
                    # Batch normalize all states together for efficiency
                    with torch.no_grad():
                        all_states = torch.cat([
                            policy_state, policy_next_state, 
                            expert_state, expert_next_state
                        ], dim=0)
                        normalized_all = self.amp_normalizer.normalize(all_states)
                        
                        # Split back to original components
                        split_sizes = [
                            policy_state.shape[0], policy_next_state.shape[0],
                            expert_state.shape[0], expert_next_state.shape[0]
                        ]
                        policy_state, policy_next_state, expert_state, expert_next_state = \
                            torch.split(normalized_all, split_sizes, dim=0)
                ...
                # Only update normalizer statistics in training mode
                if (self.normalize_amp_input and self.amp_normalizer is not None and 
                    hasattr(self.amp_normalizer, 'training') and self.amp_normalizer.training):
                    # Update normalizer with original (non-normalized) data
                    original_policy = sample_amp_policy[0]  # Use original policy state
                    original_expert = sample_amp_expert[0]  # Use original expert state
                    self.amp_normalizer.update(original_policy)
                    self.amp_normalizer.update(original_expert)

                policy_d_prob = torch.sigmoid(policy_d)
                expert_d_prob = torch.sigmoid(expert_d)

                # Original accuracy calculations (kept for backward compatibility)
                ...
                # TokenHSI-style accuracy calculations (using logits directly)
                disc_agent_acc = policy_d < 0  # Policy samples should have logit < 0 (classified as fake)
                disc_agent_acc = torch.mean(disc_agent_acc.float())
                disc_demo_acc = expert_d > 0   # Expert samples should have logit > 0 (classified as real)
                disc_demo_acc = torch.mean(disc_demo_acc.float())
                
                mean_disc_agent_acc += disc_agent_acc.item()
                mean_disc_demo_acc += disc_demo_acc.item()
                
                # Discriminator separation (expert_pred - policy_pred)
                discriminator_separation = expert_d_prob.mean() - policy_d_prob.mean()
                mean_discriminator_separation += discriminator_separation.item()
                
            else:
                # No expert data: fall back to standard PPO update.
                self.optimizer.zero_grad()
                ppo_loss.backward()
                nn.utils.clip_grad_norm_(self.actor_critic.parameters(), self.max_grad_norm)
                self.optimizer.step()
        ...
        if self.amp_data is not None:
            ...
            mean_discriminator_separation /= num_updates
