...
def process_motion(key_names, key_name_to_pkls, cfg):
    ...
    # print(f"[DEBUG] robot joint names: {humanoid_fk.joint_names}")
    ...
    for data_key in pbar:
        # print(key_name_to_pkls[data_key])
        ...
        # 关键修改：保持第一帧的朝向，去除扭腰动作
        gt_root_rot[:, 0] = 0.0  # x (pitch) 
        gt_root_rot[:, 1] = 0.0  # y (roll)
        # 保持z轴旋转为第一帧的值
        first_frame_z = gt_root_rot[0, 2].clone()
        gt_root_rot[:, 2] = first_frame_z  # z (yaw) - 保持第一帧的值
        ...
        print(f"[DEBUG] dof_pos_new shape: {dof_pos_new.shape}")  # 应该是 [1, T, num_dof, 1]
        print(f"[DEBUG] humanoid_fk.dof_axis shape: {humanoid_fk.dof_axis.shape}")  # [num_dof, 3]
        for iteration in range(cfg.get("fitting_iterations", 500)):
            # 在每次迭代中确保旋转约束
            with torch.no_grad():
                root_rot_new.data[:, 0] = 0.0  # x (pitch)
                root_rot_new.data[:, 1] = 0.0  # y (roll)
                root_rot_new.data[:, 2] = first_frame_z  # z (yaw) - 保持第一帧的值
        ...
        root_rot_fixed[:, 2] = first_frame_z.item()  # 所有帧都使用第一帧的z轴旋转
...
def main(cfg : DictConfig) -> None:
    ...
    if not cfg.get("fit_all", False):
        key_names = ["0-Female1Walking_c3d_B3 - walk1_poses"]
