...
g1_xml = "phc/data/assets/robot/g1_12/g1_12dof.xml"
g1_urdf = "phc/data/assets/robot/g1_12/g1_12dof.urdf"
asset_descriptors = [
    # AssetDesc(h1_xml, False),
    AssetDesc(g1_urdf, False),
]
sk_tree = SkeletonTree.from_mjcf(g1_xml)
motion_file = "data/g1_12/v1/singles/0-Male2Walking_c3d_B15 -  Walk turn around_poses.pkl"
...
num_motions = 1
curr_start = 0
motion_lib.load_motions(skeleton_trees=[sk_tree] * num_motions, gender_betas=[torch.zeros(17)] * num_motions, limb_weights=[np.zeros(10)] * num_motions, random_sample=True)
...
while not gym.query_viewer_has_closed(viewer):
    ...
    # 确保DOF维度匹配G1机器人的29个DOF
    if dof_pos.shape[-1] == 28:
        # 在开头添加根关节的DOF（通常设为0）
        root_dof = torch.zeros(dof_pos.shape[0], 1, device=dof_pos.device, dtype=dof_pos.dtype)
        dof_pos = torch.cat([root_dof, dof_pos], dim=-1)
    elif dof_pos.shape[-1] < 29:
        # 如果还不够29个，继续填充零
        padding = torch.zeros(dof_pos.shape[0], 29 - dof_pos.shape[-1], 
                            device=dof_pos.device, dtype=dof_pos.dtype)
        dof_pos = torch.cat([dof_pos, padding], dim=-1)
